...
class bot_pathplanner():
    ...
    def find_path_nd_display(self,graph,start,end,maze,method = "DFS"):
        ...
        if method=="DFS":
            ...
            if not self.astar.shortest_path_found:
                print("Finding Shortest ROutes")
...
class dijisktra:
    def __init__(self):
        ...
        self.idxs2vrtxs = {}
        self.vrtxs2idxs = {}
        # Counter added to track total nodes visited to reach goal node
        self.dijkstra_nodes_visited = 0

    def ret_shortestroute(self, parent, start, end, route):
        # Keep updating the shortest route from end to start by visiting closest vertices starting from end
    ...
    def find_best_routes(self, graph, start, end):
        # Taking the first item of the list created by list comprehension
        ...
        # Distance list storing dist of each node
        dist = []
        # Storing found shortest subpaths [format ==> (parent_idx = closest_child_idx)]
        parent = []
        # Set size of minHeap to be the total number of keys in the graph.
        ...
        for idx, v in enumerate(graph.keys()):
            ...
            # So instead of a vertex which is a tuple representing an IP, we pass in an index
        ...
        # Set dist of start_idx to 0 while everything else remains Inf
        dist[start_idx] = 0
        # Decrease Key as a new found dist of start_vertex is 0
        self.minHeap.decreaseKey(start_idx, dist[start_idx])

        # Loop as long as the Priority Queue has nodes.
        while self.minHeap.size != 0:
            # Counter added for keeping track of nodes visited to reach the goal node
            self.dijkstra_nodes_visited += 1
            ...
            for v in graph[u]:
                ...
                if v != "case":
                    ...
                    # If we have not found the shortest distance to v + new found dist < known dist, update dist for v
                    if (
                        self.minHeap.isInMinHeap(v_idx)
                        and dist[u_idx] != 1e7
                        and (
                            (graph[u][v]["cost"] + dist[u_idx]) < dist[v_idx]
                        )
                    ):
                        # Modify the distance calculation to include the weight
                        weight = graph[u][v]["cost"] + abs(u_idx - v_idx)
                        if weight < dist[v_idx]:
                            dist[v_idx] = weight
                            self.minHeap.decreaseKey(v_idx, dist[v_idx])
                            parent[v_idx] = u_idx

            # End Condition: When our end goal has already been visited.
            # This means the shortest path to the end goal has already been found
            # Do ---> Break Loop
            if u == end:
                break
