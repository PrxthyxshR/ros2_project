...
class maze_solver(Node):
    ...
    def maze_solving(self):
        ...
        if config.debug and config.debug_pathplanning:
            print("\nNodes Visited [Dijisktra V A-Star*] = [ {} V {} ]".format(self.bot_pathplanner.dijisktra.dijkstra_nodes_visited,self.bot_pathplanner.astar.astar_nodes_visited))
        
        if self.bot_pathplanner.dijisktra.shortest_path_found:
            self.bot_pathplanner.shortest_path_planned = True
